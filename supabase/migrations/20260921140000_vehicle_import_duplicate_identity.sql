-- =============================================================================
-- Importação de frotas: chassi e RENAVAM repetidos no arquivo
--
-- A validação conferia duplicidade dentro da planilha só para placa e código
-- de frota. Chassi e RENAVAM identificam o veículo tanto quanto a placa, e o
-- banco tem índice único para os dois — então uma planilha com o mesmo RENAVAM
-- em duas linhas passava pela validação inteira e só estourava na gravação.
--
-- É exatamente o que a área de staging existe para evitar: o erro tem de
-- aparecer na pré-visualização, com o número da linha, antes de qualquer
-- escrita. Apareceu numa base real de 95 veículos, em que duas linhas
-- compartilhavam o RENAVAM 01435172946.
--
-- A função é reemitida inteira porque é assim que o PostgreSQL substitui uma
-- função; a única mudança são os dois blocos novos na seção 2.
-- =============================================================================
